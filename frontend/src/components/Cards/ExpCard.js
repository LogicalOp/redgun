...
const ExpCard = ({ data }) => {
  ...
  const totalExperience = data
    ...
    .reduce((acc, item) => {
      ...
      return acc + (isNaN(durationInHours) ? 0 : durationInHours * experienceMultiplier);
    }, 0);
  ...
  for (let i = 0; i < experienceLevels.length; i++) {
    if (totalExperience >= experienceLevels[i].threshold) {
      ...
      nextLevelThreshold = experienceLevels[i + 1] ? experienceLevels[i + 1].threshold : experienceLevels[i].threshold;
    }
  }
  ...
  if (currentLevel > 0 && currentLevel < experienceLevels.length) {
    progressToNextLevel = ((totalExperience - experienceLevels[currentLevel - 1].threshold) / (nextLevelThreshold - experienceLevels[currentLevel - 1].threshold)) * 100;
  ...
  return (
    <Card
      header={
        <CardHeader level="H4" titleText="Experience Summary" style={{ margin: "2rem" }} />
      }
    >
      ...
      <div style={{ margin: "1rem" }}>
        <ProgressIndicator value={progressToNextLevel} displayValue={`${progressToNextLevel.toFixed(2)}%`} />
      </div>
    </Card>
  );
};
